...
PortAudioTestNode::PortAudioTestNode() {
    ...
    set_anchor(Side::SIDE_BOTTOM, 1.f);
    set_anchor(Side::SIDE_RIGHT, 1.f);
    set_anchor(Side::SIDE_TOP, .0f);
    set_anchor(Side::SIDE_LEFT, .0f);

    VBoxContainer *root = memnew(VBoxContainer);
    root->set_anchor(Side::SIDE_BOTTOM, 1.f);
    root->set_anchor(Side::SIDE_RIGHT, .0f);
    root->set_anchor(Side::SIDE_TOP, .0f);
    root->set_anchor(Side::SIDE_LEFT, .0f);
    ...
    option_host_api->connect("item_selected", callable_mp(this, &PortAudioTestNode::option_host_api_selected));
...
void PortAudioTestNode::option_host_api_selected(int p_index) {
    ...
    for (; child_index >= 0; child_index--) {
		...
		SceneTree::get_singleton()->queue_delete(child);
    ...
    scroll_device_info_collection->set_horizontal_scroll_mode(ScrollContainer::SCROLL_MODE_DISABLED);
	scroll_device_info_collection->set_vertical_scroll_mode(ScrollContainer::SCROLL_MODE_AUTO);
